...
Triangulo *Modelo::centrar(Triangulo *t){
	Vector3D centro(minX+getAncho()/2.0f,minY+getAlto()/2.0f,minZ+getProfundo()/2.0f);
	Vector3D p0=t->getP0();
	Vector3D p1=t->getP1();
	Vector3D p2=t->getP2();
...
inline Contorno *Modelo::getContorno(){
	return new Contorno(this->getP(),this->getProfundo()/2.0f);
}

//Comprobamos que si hay colision
inline bool Modelo::colision(Solido *s) {
